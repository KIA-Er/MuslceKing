...
class DataProcessor:
    ...
    def process_excel(self) -> None:
        ...
        # 遍历每个工作表并用_process_sheet() 方法进行处理
    ...
    def _process_row(
        ...
        """处理单行数据
        根据配置决定是llm生成还是直接使用扁平化处理"""
        # 将行转换为字典，key是列名,value是单元格值
        row_dict = row.to_dict()
        # 如果配置中不使用LLM，则直接扁平化处理
    ...
    def _embed_and_store(self, incremental: bool = False) -> None:
        """将处理后的数据self.processed_data进行向量嵌入并存储"""
        if not self.processed_data:
            self.logger.info("没有可入库的数据，跳过嵌入。")
            return

        items = []
        # 遍历 processed_data 中的每个字典
