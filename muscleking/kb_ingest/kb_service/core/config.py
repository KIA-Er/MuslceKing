...
class DatabaseConfig:
    """PostgreSQL with pgvector的数据库配置"""
    ...
    def from_env(cls) -> "DatabaseConfig":
        return cls(
            ...
            password=os.getenv(
                "PGPASSWORD", os.getenv("POSTGRES_PASSWORD", cls.password)
            ),
...
class Config:
    ...
    excel_file_path: str = field(
        default_factory=lambda: os.getenv("EXCEL_FILE_PATH", "data.xlsx")
    )
    output_csv_path: str = field(
        default_factory=lambda: os.getenv("OUTPUT_CSV_PATH", "processed_data.csv")
    )
    output_txt_path: str = field(
        default_factory=lambda: os.getenv("OUTPUT_TXT_PATH", "processed_data.txt")
    )

    # rewriting
    use_llm: bool = field(
        default_factory=lambda: os.getenv("USE_LLM", "true").lower() != "false"
    )
    flat_sep: str = field(default_factory=lambda: os.getenv("FLAT_SEPARATOR", " | "))
    flat_kv_sep: str = field(
        default_factory=lambda: os.getenv("FLAT_KV_SEPARATOR", ": ")
    )
    flat_max_len: int = field(
        default_factory=lambda: int(os.getenv("FLAT_MAX_LEN", "4000"))
    )
    ...
    llm_fallback_to_flatten: bool = field(
        default_factory=lambda: (
            os.getenv("LLM_FALLBACK_TO_FLATTEN", "false").lower() == "true"
        )
    )
    ...
    embedding_model: str = field(
        default_factory=lambda: os.getenv("EMBEDDING_MODEL", "qwen/qwen3-embedding-8b")
    )
    ...
    rerank_provider: str = field(
        default_factory=lambda: os.getenv("RERANK_PROVIDER", "cohere")
    )
    ...
    rerank_base_url: Optional[str] = field(
        default_factory=lambda: os.getenv("RERANK_BASE_URL")
    )
    rerank_endpoint: str = field(
        default_factory=lambda: os.getenv("RERANK_ENDPOINT", "/rerank")
    )
    ...
    rerank_max_candidates: int = field(
        default_factory=lambda: int(os.getenv("RERANK_MAX_CANDIDATES", "20"))
    )
    rerank_top_n: int = field(
        default_factory=lambda: int(os.getenv("RERANK_TOP_N", "10"))
    )
    rerank_score_fusion_alpha: Optional[float] = field(
        default_factory=lambda: (
            float(os.getenv("RERANK_SCORE_FUSION_ALPHA", "0.3"))
            if os.getenv("RERANK_SCORE_FUSION_ALPHA")
            else None
        )
    )
