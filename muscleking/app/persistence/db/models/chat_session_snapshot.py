...
from sqlalchemy import Boolean, Column, DateTime, String, Text, ForeignKey, JSON, func
...
class ChatSessionSnapshot(Base):
    ...
    state_data = Column(
        JSON,
        nullable=False,
        comment="Session state data as JSON"
    )
    ...
    title = Column(
        String(500),
        nullable=True,
        comment="Snapshot title or description"
    )
