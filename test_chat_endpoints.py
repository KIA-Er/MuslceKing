...
def test_create_session():
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/sessions", params={"user_id": "test_user_001"}
        )
        print(f"创建会话状态码: {response.status_code}")
        if response.status_code == 200:
            session_data = response.json()
            print(
                f"创建的会话信息: {json.dumps(session_data, indent=2, ensure_ascii=False)}"
            )
...
def test_send_chat_message(session_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"聊天响应: {json.dumps(chat_response, indent=2, ensure_ascii=False)}"
            )
            return True
        else:
...
def test_send_chat_without_session():
    ...
    print("\n测试不提供会话ID发送消息...")
    try:
        chat_data = {"message": "我想创建新的会话", "user_id": "test_user_002"}
        response = requests.post(f"{BASE_URL}/chat", json=chat_data)
        print(f"发送消息状态码: {response.status_code}")
        if response.status_code == 200:
            chat_response = response.json()
