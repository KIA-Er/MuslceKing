...
async def test_graph_construction():
    ...
    try:
        ...
        for i, ex in enumerate(demo_exercises[:5], 1):
            print(f"  {i}. {ex['name']} - 设备: {ex.get('equipment', [])}, 目标肌群: {ex.get('targetMuscle', [])}")
...
async def test_cypher_generation():
    ...
    description = test_exercise["instructions"][0] if test_exercise["instructions"] else ""
    equipment = [builder._normalize_equipment(e) for e in test_exercise.get("equipment", [])]
